...
// 차트용 더미 데이터에요. 실제 데이터 추가하면 지워야 합니다
const weeklyData = [
  { date: '4/1', sales: 120, revenue: 38.2 },
  { date: '4/2', sales: 132, revenue: 42.1 },
  { date: '4/3', sales: 101, revenue: 31.5 },
  { date: '4/4', sales: 134, revenue: 44.8 },
  { date: '4/5', sales: 155, revenue: 51.2 },
  { date: '4/6', sales: 139, revenue: 46.5 },
  { date: '4/7', sales: 145, revenue: 45.8 },
];

const categoryData = [
  { name: '아트', value: 420, color: '#0088FE' },
  { name: '컬렉션', value: 350, color: '#00C49F' },
  { name: '사진', value: 180, color: '#FFBB28' },
  { name: '음악', value: 120, color: '#FF8042' },
  { name: '기타', value: 90, color: '#8884d8' },
];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <StatCardV2
          ...
          label="승인 완료"
          value={nfts.filter(n => n.approval_status !== 'pending').length}
          change={'+8.5%'}
          color="emerald"
        />
      </div>
      ...
      {/* 오늘 날짜 표시 */}
      <div className="mb-2 text-right text-th-text-secondary text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div className="col-span-2 bg-white rounded-2xl shadow p-6 border border-gray-100">
          <div className="font-semibold mb-2">일주일 판매 추이</div>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={weeklyData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <Tooltip />
              <Legend />
              <Area yAxisId="left" type="monotone" dataKey="sales" stroke="#8884d8" fillOpacity={1} fill="url(#colorSales)" name="판매량" />
              <Area yAxisId="right" type="monotone" dataKey="revenue" stroke="#82ca9d" fillOpacity={1} fill="url(#colorRevenue)" name="수익 (ETH)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        {/* 카테고리별 분포 */}
        <div className="bg-white rounded-2xl shadow p-6 border border-gray-100">
          <div className="font-semibold mb-2">카테고리별 분포</div>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie
                data={categoryData}
            ...
            {categoryData.map((cat, idx) => (
              <div key={idx} className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-2">
                  <span className="inline-block w-3 h-3 rounded" style={{ background: cat.color }} />
                  {cat.name}
                </span>
                <span className="font-semibold">{cat.value}개</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow p-6 border border-gray-100 mb-10">
        <div className="font-semibold mb-2">시간대별 거래량</div>
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={hourlyData}>
            ...
            <YAxis />
            <Tooltip />
            <Bar dataKey="count" fill="#8884d8" name="거래 수" />
          </BarChart>
        </ResponsiveContainer>
      </div>
...
// 피그마 스타일 통계 카드
function StatCardV2({ icon, label, value, change, color }) {
  const colorMap = {
    yellow: {
      bg: 'bg-yellow-50',
  ...
  return (
    <div className={`rounded-2xl p-6 border ${c.bg} ${c.border} shadow-sm flex flex-col gap-2`}>
      <div className="flex items-center justify-between mb-2">
        <span className={`inline-flex items-center justify-center w-10 h-10 rounded-xl ${c.chip}`}>{icon}</span>
        <span className={`text-xs px-2 py-1 rounded-full font-semibold ${c.chip}`}>{change}</span>
      </div>
      <div className="flex-1">
        <div className={`text-sm font-medium mb-1 ${c.text}`}>{label}</div>
